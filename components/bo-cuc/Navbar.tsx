...
import SyncNotification from '../giao-dien/dong-bo/SyncNotification';
    ...
    <nav className={`fixed top-0 h-14 flex items-center justify-between px-4 bg-gradient-to-r from-slate-900/95 via-slate-800/95 to-slate-900/95 dark:from-gray-900/95 dark:via-gray-800/95 dark:to-gray-900/95 backdrop-blur-lg border-b border-slate-600/30 dark:border-gray-600/30 z-40 navbar shadow-lg transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'md:left-16 md:w-[calc(100vw-4rem)]' : 'md:left-64 md:w-[calc(100vw-16rem)]'} left-0 w-full`}>
      <div className="flex items-center gap-3">
        ...
        <div className="md:hidden flex items-center gap-3 cursor-pointer select-none" onClick={onBrandClick} title="Support HR">
          <div className="w-9 h-9 rounded-lg overflow-hidden border border-white/10 bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center">
            <img
              src="/images/logos/logo.jpg"
              alt="Support HR"
              className="object-contain w-full h-full p-1"
              onError={(e) => { const t=e.currentTarget as HTMLImageElement; t.style.display='none'; (t.parentElement as HTMLElement).innerHTML='<span class=\"font-bold text-sm text-white\">S</span>'; }}
              draggable={false}
            />
          </div>
          <span className="text-lg font-bold tracking-wide bg-gradient-to-r from-blue-400 via-indigo-400 to-pink-400 bg-clip-text text-transparent">Support HR</span>
        </div>
      </div>
        ...
        {userEmail ? (
          ...
          onLoginRequest && (
            <button
              ...
              className="px-4 py-2 rounded-md bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-sm font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400/40"
            >
